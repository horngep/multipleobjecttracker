...
rho1 = 7 ; sigma1 = 2; % to detect small bubbles
rho2 = 17; sigma2 = 2;
rho3 = 27; sigma3 = 2; % to detect big bubbles
rho4 = 40; sigma4 = 2; 
rho5 = 12; sigma5 = 2;
...
for cir = 1:length(cir_type)
    ...
    T1 = create_template(rho1,patchSize,sigma1, cir_type{cir});
    template_list{length(template_list)+1} = T1;
    T2 = create_template(rho2,patchSize,sigma2, cir_type{cir});
    template_list{length(template_list)+1} = T2;
    T3 = create_template(rho3,patchSize,sigma3, cir_type{cir});
    template_list{length(template_list)+1} = T3;
    T4 = create_template(rho4,patchSize,sigma4, cir_type{cir});
    template_list{length(template_list)+1} = T4;
    T5 = create_template(rho5,patchSize,sigma5, cir_type{cir});
    template_list{length(template_list)+1} = T5;
...
%for i = 1:2:128
    ...
    for ncc = 1:length(template_list)
        ...
        nc = nc(pso2:end-pso2, pso2:end-pso2); % get rid of augmented boundary
