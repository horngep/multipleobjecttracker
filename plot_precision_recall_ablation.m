function [ pr_labels,pr_scores ] = plot_precision_recall_ablation( labeled_total_acc_bub_list,GT )
%PLOT_PRECISION_RECALL_AFLATION Summary of this function goes here
...
length(labeled_total_acc_bub_list);


% 1.1 determining inputs ('pr_labels' and 'pr_scores')

% 1.1.1 getting sorted labeled_total_acc_bub_list to decending order of max_ncc_score score
len = length(labeled_total_acc_bub_list);

% we only uses unode(col 1) to identify bubble and max_ncc_val(col 2) to sort them
a = zeros(len,2); 

for i = 1:len
    b = labeled_total_acc_bub_list{i}; 
    a(i,1) = double(b.unode);
    a(i,2) = double(b.max_ncc_val);
end
...
res = {}; % all the bubbles with sorted max_ncc_val
for i = 1:len
    unode = a_sorted(i,1);
...
for i = 1:len
    ...
    for j = 1:length(bubs_this_frame)
        ...
        overlap_threshold = 0.6; % >> VARIABLE: overlapping threshold <<
...
for i = 1:len
    ...
    pr_scores(1,i) = res{i}.max_ncc_val;
end
