...
% Augment interp mode to say samples are evenly spaced.
...
if 1
  ...
  A = inv(H)*[U(:) V(:) ones(prod(size(U)),1)]';
  s = 1./A(3,:);
  U(:) = A(1,:).*s;
  V(:) = A(2,:).*s;
end
...
if strcmp(interp_mode, '*linear')
  if verbose, tic, end
  locprintf(verbose, 2, 'warping... ');
  if 0
     if l == 3
        nim(nrows, ncols, 3) = 1;
        locprintf(verbose, 2, '[R ');
        nim(:,:,1) = vgg_interp2(im(:,:,1),U,V,interp_mode);
        locprintf(verbose, 2, 'G ');
        nim(:,:,2) = vgg_interp2(im(:,:,2),U,V,interp_mode);
        locprintf(verbose, 2, 'B] ');
        nim(:,:,3) = vgg_interp2(im(:,:,3),U,V,interp_mode);
     else  
        nim(nrows, ncols) = 1;
        nim(:,:) = vgg_interp2(im,U,V,interp_mode);
     end  
  else
     nim = vgg_interp2(im,U,V,interp_mode);
  end
  %locprintf(verbose, 2, '[%dms] ', round(toc * 1000));
  locprintf(verbose, 2, 'done\n');
else
  if l == 3
    nim(nrows, ncols, 3) = 1;
    locprintf(verbose, 2, '[R ');
